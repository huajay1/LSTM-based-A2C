...
class EnvMove(object):
    ...
    def user_move(self):
        ...
        UE_index = np.where(self.UE_pos[:, 1] < -3 * self.BS_radius)
        self.UE_pos[UE_index, 1] = -6 * self.BS_radius - self.UE_pos[UE_index, 1]
        self.UE_direction[UE_index] = -self.UE_direction[UE_index]
        ...
        self.volte_dis = n1 / tmp_u.shape[0]

        tmp_u = self.UE_pos[np.where(self.UE_cat == 'embb_general')]
        tmp_dis = np.sum(tmp_u ** 2, axis=1)
        n2 = np.sum(tmp_dis <= (4 * self.BS_radius / 4) ** 2)
        self.embb_dis = n2 / tmp_u.shape[0]

        tmp_u = self.UE_pos[np.where(self.UE_cat == 'urllc')]
        tmp_dis = np.sum(tmp_u ** 2, axis=1)
    ...
    def activity(self):  # https://www.ngmn.org/fileadmin/user_upload/NGMN_Radio_Access_Performance_Evaluation_Methodology.pdf
        ...
        ##---------------------------------------------------------------##
            if self.UE_buffer[:, ue_id].size - np.count_nonzero(
                    self.UE_buffer[:, ue_id]) != 0:  # The buffer is not full
    ...
    def store_reward(self, rate):
        ...
        handling_latency = 0
        ##-----------------------------------------------
        UE_index = np.where(self.UE_cell == 1)[0]
                    ...
                    if self.UE_cat[ue_id] == 'volte':
                        ...
                        if (self.UE_latency[i, ue_id] == self.time_subframe):
                            if (rate[ue_id] >= 10 * 10 ** 6) & (
                                    self.UE_latency[i, ue_id] < 3 * 10 ** (-3) - handling_latency):
                                self.succ_tx_pkt_no[cat_index] += 1
                        else:
    ...
    def get_reward(self):
        se_total = self.sys_se_per_frame / (self.learning_windows / self.time_subframe - self.idle_frame)
